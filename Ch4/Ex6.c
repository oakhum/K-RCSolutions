/* K&R Ex. 4-6: The calculator program with commands for handling 
	variables, and a variable for the most recently printed value. */
...
main()
{
	...
	while ((type = getop(s)) != EOF) {
		switch (type) {
			...
			case LETTER: push(calcfunctions(s)); break;
...
int getop(char s[])
{
	...
	s[i] = '\0';
	if (c != EOF) ungetch(c);
	return (isalpha(s[0])) ? LETTER : NUMBER;
}
